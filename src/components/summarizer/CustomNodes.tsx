...
export const TopicNode = ({ data }: { data: any }) => (
  <div className="relative px-4 py-2 bg-blue-500 text-white rounded-lg font-semibold min-w-[120px] text-center">
...
export const TagNode = ({ data }: { data: any }) => (
  <div className="relative px-3 py-2 bg-green-500 text-white rounded-md font-medium min-w-[100px] text-center">
    {data.label}
    <div className="text-xs opacity-80">{data.count} notes</div>
...
export const NoteNode = ({ data }: { data: any }) => (
  <div
    className={`relative px-3 py-2 rounded-md cursor-pointer transition-all min-w-[150px] ${
  ...
  >
    ...
    {/* Leaf nodes only receive connections from the left */}
    <Handle type="target" position={Position.Left} id="left" />
  </div>
);
